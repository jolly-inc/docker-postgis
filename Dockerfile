...
RUN service postgresql start && /bin/su postgres -c "createuser -d -s -r -l jolly" \
    && /bin/su postgres -c "psql postgres -c \"ALTER USER jolly WITH ENCRYPTED PASSWORD 'jolly'\"" \
